...
if __name__ == '__main__':
    ...
    foo_server = Server(DEFAULT_HOST, FOO_SERVER_PORT)
    foo_server.connect()
    foo_server.log_mesage("Connected foo server")

    foo_client = Client(DEFAULT_HOST, FOO_CLIENT_PORT)
    try_client_connection(foo_client)
    foo_client.log_mesage("Connected foo client")

    with ThreadPoolExecutor(max_workers=2) as executor:
        executor.submit(send_client_mesage, b'I am foo', 
            foo_client, SERVER_PORT)
        
        executor.submit(receive_client_message, foo_server)
